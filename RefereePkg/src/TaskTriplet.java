...
{	
	...
	private void getOrientationFromUser() throws Exception
	{
		...
		try
		{
			...
			do
			{
				...
				{
					System.out.println( "Orientation " + s + " is invalid. Retry.");
					repeatLoop = true;
				}
			} while ( repeatLoop );
		...
		{
			System.out.println ( "While reading Orientation from user: " + e.getMessage() );
			throw e;
		}
	}
	...
	private void getPauseFromUser() throws Exception
	{
		...
		{
			System.out.println ( "While reading Pause from user: " + e.getMessage() );
			throw e;
		}
	}
	...
	public boolean setTaskTriplet( String tripletString )
	{
	...
	/**
	 This method reads a task triplet from the user and stores in the invoking object.
	*/
	public void getTaskTripletFromUser() throws Exception
	{
		boolean repeatLoop = false;
		try
		{
			...
			{
				repeatLoop = false;
				ValidTripletElements.displayValidTripletElements(); //To assist user in entering valid task specification.
				System.out.println( "Enter triplet as (PLACE, ORIENTATION, PAUSE) or enter PLACE element of triplet: " );
				String s = br.readLine();
				if ( s.charAt( 0 ) == '(' )
				{
					if ( s.matches( "(\\([^,]+\\,[^,]+\\,[^,]+\\))" ) )
					{
						System.out.println( "User has entered a triplet. Checking for validity..." );
						if ( setTaskTriplet( s ) )
						{
							System.out.println( "Triplet " + s + " is valid."); 
						}
						else
						{
							System.out.println( "Triplet " + s + " is invalid. Retry." );
							repeatLoop = true;							
						}
					}
					else
					{
						System.out.println( "Input format not recognised. Retry." );
						repeatLoop = true;
					}
				}
				else
				{
					System.out.println( "User appears to have entered place. Checking for validity..." );
					if ( setPlace( s ) )
					{
					...
					{
						System.out.println( "Place " + s + " is invalid. Retry." );
						repeatLoop = true;
					}
				}
			} while ( repeatLoop );		
		...
		{
			System.out.println( "Caught exception in getTaskTripletFromUser: " + e.getMessage() );
			throw e;
		}
	}
